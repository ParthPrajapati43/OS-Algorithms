...
def solvePriorityIO(self):
    ...
    print("JobNumber: " + str(jobNumber))
    print("PriorityList: " + str(Prioritylist))
    ...
    while(completed != totalJobs):
        ...
        for i in range(len(ReadyTemp)):
            print(Prioritylist[ReadyTemp[i]])
        # sorting the ready Queue according to priority
        for i in range(len(ReadyTemp)):
            for j in range(i, len(ReadyTemp)):
                if(Prioritylist[ReadyTemp[i]] > Prioritylist[ReadyTemp[j]]):
                    ReadyTemp[i], ReadyTemp[j] = ReadyTemp[j], ReadyTemp[i]

        # sorting the IO queue according to priority
        for i in range(len(IOTemp)):
            for j in range(i, len(IOTemp)):
                if(Prioritylist[IOTemp[i]] > Prioritylist[IOTemp[j]]):
