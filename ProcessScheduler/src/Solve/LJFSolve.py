...
def solveLJF(self):
    ...
    while(finished != totalJobs):
        ...
        # now get the Job with the highest BT
        present = available[0]
        BT = BTlist[available[0]]
        for i in range(len(available)):
            if((BTlist[available[i]] > BT) or (BTlist[available[i]] == BT and jobNumber[available[i]] < jobNumber[present])):
                BT = BTlist[available[i]]
                present = available[i]
